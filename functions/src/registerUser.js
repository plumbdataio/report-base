const admin = require('./shared/firebase-admin.js')
let getResponseFunc = require('./shared/response.js')

exports.handler = async (event) => {
  console.log(`headers: ${JSON.stringify(event)}`);
  response = getResponseFunc(event.headers.host, event.headers.origin)

  // CORS preflight request first pass "OPTIONS" method, and we need to handle this manually.
  if(event.httpMethod === "OPTIONS")
    return response(200, "")

  if( ! event.headers.authorization )
    return response(400, "No authorization header received.")

  const idToken = event.headers.authorization
  const newUser = JSON.parse(event.body).newUser
  // console.log(`newUser: ${JSON.stringify(newUser)}`)

  let role
  await admin.auth().verifyIdToken(idToken).then(decodedIdToken => {
    // console.log(`decodedIdToken: ${JSON.stringify(decodedIdToken)}`);
    role = decodedIdToken.role ? decodedIdToken.role : null
    // console.log(`role: ${role}`)
  })
  if( ! role || role !== "admin" )
    return response(401, "Requester doesn't have permission.")

  if(role === "admin") {
    let uid
    let errorReason = null
    // Add firebase.auth() user
    await admin.auth().createUser(newUser.authData)
                      .then(userRecord => {
                        console.log(`### User successfully created: ${JSON.stringify(userRecord)}`)
                        uid = userRecord.uid
                      }, reason => {
                        errorReason = reason
                        console.log(`### Error !!! reason: ${JSON.stringify(reason)}`);
                      })
    if(errorReason !== null) {
      return response(400, errorReason.message)
    }

    // Set custom claims
    await admin.auth().setCustomUserClaims(uid, newUser.customClaims)
                      .then(() => {
                        console.log(`### Custom claims successfully set: ${JSON.stringify(newUser.customClaims)}`)
                      }, reason => {
                        errorReason = reason
                        console.log(`### Error !!! reason: ${JSON.stringify(reason)}`);
                      })
    if(errorReason !== null) {
      return response(400, errorReason.message)
    }

    const updates = {}
    updates[`/staff/${uid}`] = newUser.rtdbData
    updates[`/largestId/user/${newUser.rtdbData.affiliation}`] = uid

    // Add user to realtime database
    await admin.database().ref().update(updates).then(val => {
      console.log(`### Staff data sccessfully registered: ${val}`);
    }, reason => {
      errorReason = reason
      console.log(`### Error !!! reason: ${JSON.stringify(reason)}`);
    })

    return response(200, "### User successfully created!!! ###")
  }
}