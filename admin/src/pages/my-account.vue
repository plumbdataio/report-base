    ...
    <b-container class="p-0 grand-container">
      <b-row>
        <b-col>
          ...
          <div class="avoid-text-cls">{{this.staffName}}</div>
        </b-col>
      </b-row>
      <br>
      <b-row>
        <b-col>
          ...
          <div class="avoid-text-cls">{{this.companyName}}</div>
        </b-col>
      </b-row>
      <br>
      <b-row align-v="center">
        <b-col cols="auto">
          ...
          <div class="avoid-text-cls">{{$store.getters.loginUser.email}}</div>
        </b-col>
        <b-col>
...
export default {
  created() {
    this.$firebase.db.fetchMyData(this.$store.getters.loginUser.uid)
    .then(data => {
      if( data == null ){
        this.isMyDataFetched = false
        return alert('一部のアカウント情報が取得できませんでした。')
      }
  ...
  data() {
    return {
      ...
      recaptchaVerifierObject: null,
    }
  },
  validations: {
    emailNew: {
      required: requiredIf(function(){
        return this.isEdittingEmail
      }),
      email
    },
    passwordOld: {
    ...
    passwordRepeated: {
      ...
      sameAsPassword: sameAs(function() {return this.passwordNew})
    }
  },
  methods: {
    toggle() {
    ...
    async changePassword() {
      if(this.passwordNew !== this.passwordRepeated) {
        return alert("新しいパスワードが一致していません。")
      }
      ...
      .then(() => {
        ...
        .then(() => {
          ...
          if(error.code?.includes("auth/weak-password")) {
            alert(`エラー：新しいパスワードが要件を満たしていません。\n ${JSON.stringify(error)}`)
          } else {
            alert(`エラー：何らかの理由でパスワードの変更に失敗しました。\n ${JSON.stringify(error)}`)
          }
        }).finally(() => this.isChanging = false)
      }).catch(error => {
        console.log(error);
        if(error.code?.includes("auth/wrong-password")) {
          alert(`エラー：古いパスワードが間違っています。\n Error: ${JSON.stringify(error)}`)
        } else {
          alert(`エラー：何らかの理由でパスワードの変更に失敗しました\n Error: ${JSON.stringify(error)}`)
        }
