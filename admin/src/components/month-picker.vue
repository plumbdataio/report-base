...
<div class="grand-container">
  <b-input :disabled="disabled" size="sm" class="readonly-input" :readonly="! disabled" placeholder="(選択されていません)" @click="reverse" :value="value ? displayedMonth : null"></b-input>
...
export default {
  ...
  data() {
    return {
      ...
      selectedYear: parseInt(this.value?.split('-')[0])
    }
  },
  watch: {
    isMonthListShown: {
  ...
  computed: {
    valueYear() { return parseInt(this.value?.split('-')[0]) },
    valueMonth() { return parseInt(this.value?.split('-')[1])},
    displayedMonth() {
  ...
  methods: {
    ...
    select(month) {
      this.$emit('input', this.$dayjs().year(this.selectedYear).month(parseInt(month)-1).format(this.outputFormat))
      this.reverse()
    },
